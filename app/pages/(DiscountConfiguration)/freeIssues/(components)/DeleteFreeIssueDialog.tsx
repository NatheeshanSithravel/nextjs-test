...
import {  deleteDiscount, deleteFreeIssue } from '@/app/(services)/SFARestClient';
...
export default function DeleteFreeIssueDialog({ freeIssue, setReload, reload }: Props) {
    ...
    const onDelete = async () => {
        ...
        try {
            const res = await deleteFreeIssue(token, req);
            if (res?.success) {
                setAlert({
                    open: true,
                    type: "success",
                    message: "Free Issue deleted successfully",
                });
            } else {
                setAlert({
                    open: true,
                    type: "error",
                    message: "Failed to delete the Free Issue",
                });
            }
        } catch (error) {
            setAlert({
                open: true,
                type: "error",
                message: "An error occurred while deleting the Free Issue",
            });
        } finally {
            setLoading(false);
            ...
            <Dialog open={open} onClose={closeDialog} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Stack>
                        ...
                        Are you sure, you want to remove the {freeIssue?.freeIssueName} ?
