...
import { productCode, selectAllDiscountByPro, } from '@/app/(services)/SFARestClient';
...
export default function Page() {
    ...
    const [disDiscountQuantList, setdisDiscountQuantList] = useState<any[]>([]);
    ...
    /////////////////Methods///////////////
    const selectDiscount = async () => {
        const req = {
            tokenString: token,
            productCode: pcode
        }

        const res = await selectAllDiscountByPro(token, req);

        if (res?.success) {
            setdisDiscountQuantList(res.discountQuantList);
        } else {
            setdisDiscountQuantList([]);
        }
    }
    ...
    return (
        <PageContent title="Discounts" action={(role == UserRole.COOP_ADMIN || role == UserRole.COORDINATOR) && <DiscountDialog
            reload={reload}
            setReload={setReload}
            create={true}
            disProductList={productList}
        />}>
            ...
            <TableContainer>
                <Table>
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell><strong>Percentage Type</strong></TableCell>
                    ...
                    <TableBody>
                        {disDiscountQuantList
                            ?.slice(tablePage * rowsPerPage, tablePage * rowsPerPage + rowsPerPage)
                            .map((row: any, index) => (
                                <TableRow key={index}>
                                    <TableCell>{row.productCode}</TableCell>
                                    <TableCell>{row.discountName}</TableCell>
                                    <TableCell>{row.percentageType}</TableCell>
                ...
                <TablePagination
                    component="div"
                    count={disDiscountQuantList?.length || 0}
                    page={tablePage}
