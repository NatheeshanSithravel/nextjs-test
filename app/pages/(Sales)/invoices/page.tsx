...
import { addProductCategory, addProductRequest, findAllInvoices, findAllOrders, findDistributors, findIteneryList, getGINByUser, productCode, retrieveAllProductCategories, retriewAllDistributors, retriewAllProducts, retrivewDistributors, } from '@/app/(services)/SFARestClient';
...
export default function Product() {
    ...
    const [globalInvoiceDtoList, setglobalInvoiceDtoList] = useState<any[]>([]);
    ...
    const retriewInvoiceList = async () => {
        const isPaymentDue = status === "1" ? true : status === "0" ? false : null;
        ...
        const invoiceRetriveResponce = await findAllInvoices(token, tempRequest);

        if (invoiceRetriveResponce?.success === true) {
            setglobalInvoiceDtoList(invoiceRetriveResponce.invoiceDtoList);
        } else {
            setglobalInvoiceDtoList([]);
            setAlert({
                open: true,
                type: "error",
    ...
    return (
        <PageContent title="Invoices" action={
            <InvoiceDialog
                reload={reload}
                setReload={setReload}
                create={true}
            ...
            <Grid2 container width={'100%'} spacing={2} sx={{ alignItems: 'center' }} mb={2}>
                ...
                <Grid2 size={{ xs: 12, md: 3 }}>
                    <FormControl fullWidth size="small">
                        ...
                        >
                            <MenuItem value={"0"}>Payment Completed</MenuItem>
            ...
            <TableContainer>
                <Table>
                    ...
                    <TableBody>
                        {globalInvoiceDtoList
                            ?.slice(tablePage * rowsPerPage, tablePage * rowsPerPage + rowsPerPage)
                            .map((row: any, index) => (
                                <TableRow key={index}>
                                    <TableCell>{row.invoice.invoiceId}</TableCell>
                                    <TableCell>{row.invoice.itineraryId}</TableCell>
                                    <TableCell>{row.invoice.status}</TableCell>
                                    <TableCell>{row.invoice.totalAmount}</TableCell>
                                    <TableCell>
                                        <ViewInvoiceDialog selectedInvoice={row} />
                                    </TableCell>
                                </TableRow>
                            ))}
                ...
                <TablePagination
                    component="div"
                    count={globalInvoiceDtoList?.length || 0}
                    page={tablePage}
